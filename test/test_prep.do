* (This is meant to be run WITHOUT estdb installed)
...
* Run regressions and add results to db
	...
	estdb add: reg price weight
	estdb add: reg weight price
	estdb add, notes(model=ols smpl=2 vars=all): reg price length
	estdb add, prefix("bar"): reg price head length
	local fn = e(filename)
